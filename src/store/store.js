...
export let store = new Vuex.Store({
  ...
  mutations: {
    setPostData(state, param){
      state.postdata = param;
    },
    setName(state, param){
      state.userName = param;
    },
    setImage(state, param){
      state.userImage = param;
    }
  },
  //[리액트] => redux 와 동일
  actions : {
    async loadPostData(context){
      /* eslint-disable */
      /*version 1 : axios로 ajax 호출 (callback then 사용)
      ...
      var result = await axios.get('https://vuestargram-39e5c.firebaseio.com/postdata.json?orderBy="id"&limitToLast=3');
      //최신 버전 
      var showPostList = Object.values(result.data)
                               .reverse();
      console.log(showPostList);
      //commit을 호출해서 set을 해주어야 한다.
      context.commit("setPostData", showPostList);
    },
    ...
    //1. ajax 요청을 통해 mutations를 해야 되는 경우
      ...
      console.log(result.data);
      //commit을 호출해서 set을 해주어야 한다.
      context.commit("setName", result.data);
      // 사용법 : $store.dispatch("loadName")
    }
  }  
})
